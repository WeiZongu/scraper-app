"""
検索キーワードからWebを検索し、各ページのテキストからAI（Gemini API）で
抽出項目（列）を読み取り、行データとして抜き出すエンジン。
各抽出項目は「タイトル（表の列名）」と「キーワード（何を探すかの説明。
自然言語のヒントでよく、"*" なら列名から連想される情報を自由に拾う）」の組で、
抽出項目がそのまま表の列になり、各行はタイトルごとの値を持つ key:value の
組み合わせになる。

以前はDOMの構造とキーワードの完全一致・ワイルドカードに頼るヒューリスティック
方式だったが、サイトによって表記ゆれや情報の配置に弱く抽出精度が低かったため、
AIによる意味理解ベースの抽出（Google AI StudioのGemini API。クレジットカード
登録不要の無料枠で利用可能）に切り替えた。
...
from .ai_extract import AiExtractError, check_api_key, extract_records
...
# 検索キーワードを常にこれらの言語にも翻訳し、多言語で検索する
...
# ブラウザ内(JS)で実行し、ページの可視テキスト全体（AIへの入力用）と、
# ページを代表する画像・動画のURL（PowerPoint出力の背景・リンク用）を取得する。
# 抽出項目の判定自体はAI（Gemini）が行うため、ここではDOM構造やキーワード
# 一致は一切見ない。画像はロゴ・アイコンらしきものやヘッダー/フッター内は
# 除外し、本文中の最初の画像を代表として使う。
_PAGE_TEXT_AND_MEDIA_JS = """
(() => {
    const text = (document.body ? document.body.innerText : "") || "";

    const iconPattern = /logo|icon|sprite|pixel|spacer|tracking|avatar/i;
    let imageUrl = "";
    for (const img of document.querySelectorAll("img[src]")) {
        if (img.closest("header, nav, footer")) continue;
        if (iconPattern.test(img.src)) continue;
        imageUrl = img.src;
        break;
    }
    ...
    return {text: text, imageUrl: imageUrl, videoUrl: videoUrl};
})();
"""


def _get_page_text_and_media(page: Page) -> dict:
    return page.evaluate(_PAGE_TEXT_AND_MEDIA_JS)
...
def _consolidate_near_duplicates(
    ...
    同じ出典ページ（url）から得られた行同士は、たとえ数値の桁数の形が
    一致していても意図的にマージしない（AIが1ページから複数の別々の
    レコード、例えば複数のフライト候補や商品を正しく見つけた場合、数値
    部分だけを見ると偶然同じ形になることがあるが、それらは表記ゆれでは
    なく別々の実体であるため、平均化してしまうと値が壊れる）。
    """
    ...
    for key in order:
        group = groups[key]
        # グループ内の全行が別々のurl（1URLにつき1行）である場合のみマージ対象と
        # する。同じurl内に複数行がある場合、それらは同一ページ内でAIが見つけた
        # 別々のレコードである可能性が高く、誤って平均化しないようスキップする。
        if len(group) == 1 or len({r.get("url", "") for r in group}) != len(group):
...
def _translate_variants(
    ...
    """検索キーワードを指定言語に翻訳し、[元のキーワード, 翻訳後の表記, ...] の
    リストを返す。日本語以外のページも検索できるようにするための処理
    （抽出はAIが多言語のページ内容を直接理解するため、この翻訳は検索キーワード
    にのみ使う）。
...
def scrape(
    ...
    """
    検索キーワードでWebを検索し、上位ページのテキストをAI（Gemini）に渡して
    抽出項目（列）を読み取らせ、レコードを集める。一致しなかった列は空欄になる。
    行データのリストを返す。
    """
    fields = [
    ...
    # AI抽出のAPIキー未設定などで早期に失敗する場合、検索やブラウザ起動の
    # 無駄を避けるため、他の処理より先に確認しておく。
    try:
        check_api_key()
    except AiExtractError as e:
        raise ScrapeError(str(e)) from e

    # 検索キーワードを固定の対象言語に翻訳し、それぞれの言語で個別に検索して
    # URLを集める（検索エンジンは基本的に検索キーワードと同じ言語のページを
    # 優先的に返すため）。抽出項目自体の翻訳は、AIが多言語のページも意味理解
    # ベースで読み取れるため不要。
        ...
        try:
            for i, url in enumerate(urls, start=1):
                ...
                try:
                    ...
                    try:
                        page_data = _get_page_text_and_media(page)
                    except Exception as e:
                        on_progress(f"  -> ページ内容の取得に失敗: {e}")
                        continue

                    try:
                        records = extract_records(fields, page_data.get("text", ""), on_progress)
                    except AiExtractError as e:
                        on_progress(f"  -> AI抽出に失敗: {e}")
                        continue

                    added = 0
                    for rec in records:
                        columns = {t: rec.get(t, "") for t in column_titles}
                        if not any(columns.values()):
                            continue  # 全項目が空のレコードは無視する
